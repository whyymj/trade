...
class MarketRepo:
    ...
    def save_macro_data(self, df: pd.DataFrame) -> int:
        ...
        for _, row in df.iterrows():
            params_list.append(
                (
                    ...
                    row.get("unit"),
                    row.get("source"),
                    row.get("publish_date") if pd.notna(row.get("publish_date")) else None,
        ...
        try:
            ...
            print(f"[MarketRepo] save_macro_data error: {e}")
            return 0
    ...
    def get_macro_data(
        ...
        if indicator:
            ...
            conditions.append("publish_date >= DATE_SUB(CURDATE(), INTERVAL %s DAY)")
            params.append(days)
        ...
        FROM macro_data
        WHERE {where}
        ORDER BY publish_date DESC, period DESC
        """
    ...
    def get_latest_money_flow(self) -> Optional[dict]:
        ...
        return {
            ...
            "north_money": float(row.get("north_money")) if row.get("north_money") else None,
            "north_buy": float(row.get("north_buy")) if row.get("north_buy") else None,
            "north_sell": float(row.get("north_sell")) if row.get("north_sell") else None,
            "main_money": float(row.get("main_money")) if row.get("main_money") else None,
            "margin_balance": float(row.get("margin_balance")) if row.get("margin_balance") else None,
        }
    ...
    def get_latest_sentiment(self) -> Optional[dict]:
        ...
        return {
            ...
            "turnover_rate": float(row.get("turnover_rate")) if row.get("turnover_rate") else None,
            "advance_count": int(row.get("advance_count")) if row.get("advance_count") else 0,
            "decline_count": int(row.get("decline_count")) if row.get("decline_count") else 0,
        }
    ...
    def get_global_macro(self, symbol: str = None, days: int = 30) -> Optional[pd.DataFrame]:
        """获取全球宏观数据"""
        if symbol:
            sql = """
    ...
    def get_latest_global(self) -> Optional[List[dict]]:
        ...
        for row in rows:
            result.append({
                ...
                "change_pct": float(row.get("change_pct")) if row.get("change_pct") else None,
            })
    ...
    def get_market_features(self, days: int = 30) -> Optional[pd.DataFrame]:
        """获取合并后的市场特征（包含所有市场数据）"""
        money_df = self.get_money_flow(days)
        sentiment_df = self.get_sentiment(days)
        global_df = self.get_global_macro(days=days)

        if money_df is None and sentiment_df is None and global_df is None:
            return None

        features = pd.DataFrame()

        # 以 sentiment 为基础（包含 trade_date）
        ...
        if money_df is not None:
            if features.empty:
                features = money_df.copy()
            else:
                # 选择需要的列进行合并
                money_cols = ["trade_date", "north_money", "north_buy", "north_sell", "main_money", "margin_balance"]
                money_subset = money_df[[c for c in money_cols if c in money_df.columns]]
                features = features.merge(money_subset, on="trade_date", how="left")

        # 合并全球宏观数据（按日期 pivot）
        if global_df is not None and not global_df.empty:
            # 将 symbol 作为列名
            global_pivot = global_df.pivot_table(
                index="trade_date", 
                columns="symbol", 
                values="close_price",
                aggfunc="last"
            ...
            # 重命名列，添加 prefix
            global_pivot.columns = [f"global_{c}" if c != "trade_date" else c for c in global_pivot.columns]
            
            if features.empty:
                features = global_pivot
            else:
                features = features.merge(global_pivot, on="trade_date", how="left")
