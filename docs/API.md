...
训练并保存到数据库 `lstm_model_version` 表（只保留最新 1 个版本）。支持 SHAP、预测 vs 实际曲线图。响应中含 `metadata.version_id`、可选 `validation`: { "deployed", "reason", "new_holdout_metrics", "old_holdout_metrics" }。训练失败会记录到 `training_failure_log`，供告警（失败 ≥3 次）使用。
