...
class FundFetcher:
    ...
    def fetch_fund_nav(self, fund_code: str, days: int = 365) -> Optional[pd.DataFrame]:
        """获取基金净值历史，自动尝试多个数据源"""
        ...
        sources = [
            ("东方财富API", self._fetch_from_eastmoney),
            ("天天基金", self._fetch_from_tiantian),
    ...
    def _fetch_from_tiantian(self, fund_code: str, days: int) -> Optional[pd.DataFrame]:
        """从天天基金获取"""
        try:
            import time
    ...
    def _fetch_from_akshare(self, fund_code: str, days: int) -> Optional[pd.DataFrame]:
        """从akshare获取"""
        try:
            ...
            for col in nav_cols:
                ...
                try:
                    ...
                    accum_nav = (
                        df.iloc[0][accum_col] if accum_col in df.columns else None
                    )
    ...
    def fetch_fund_info(self, fund_code: str, timeout: int = 3) -> Optional[dict]:
        """获取基金基本信息"""
        try:
